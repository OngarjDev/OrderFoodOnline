...
class customer
{
    ...
    public function CheckItem(int $IdFood): String
    {
    ...
    public function CheckShop($IdShop){
        ...
        $cartItems = array_column($DeCart, 'IdShop');
        if (in_array($IdShop, $cartItems)) {
            return true;
        } else {
            return false;
        }
    ...
    public function AddOrder($request)
    {
        ...
        $this->service->InsertTable("orders", "IdCustomer,FoodOrder,PriceOrder,StatusOrder,DateOrder,IdShop", "'{$_SESSION['IdUser_Session']}','$jsonCartData',$sum,0,'$currentDate',{$FoodInfo['IdShop']}", null);
        setcookie('cart', '', 0, '/');
